...
class Grid:
    def __init__(self, size: Tuple[int, int]):
        self.size = size
        self.cell_size = 50  # Adjust the cell size here
    ...
    def draw(self, screen):
        for x in range(0, self.size[0] * self.cell_size, self.cell_size):
            pygame.draw.line(screen, (0, 0, 0), (x, 0), (x, self.size[1] * self.cell_size))
        for y in range(0, self.size[1] * self.cell_size, self.cell_size):
            pygame.draw.line(screen, (0, 0, 0), (0, y), (self.size[0] * self.cell_size, y))
